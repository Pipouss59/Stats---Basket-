...
<title>Stats Basket — Outil Pro</title>
...
<div class="container">
  <div class="header">
    <h1>🏀 Stats Match — Outil Pro</h1>
    <div class="controls">
      <button class="btn primary" id="saveBtn">Sauvegarder</button>
      <button class="btn accent" id="summaryBtn">Résumé</button>
  ...
  <div class="top-card row" style="margin-top:12px">
    <div class="card" style="flex:1">
      <div class="team-header">
        ...
        <input id="teamAName" class="input" value="Équipe A">
      </div>
    ...
    <div class="card" style="flex:1">
      <div class="team-header">
        ...
        <input id="teamBName" class="input" value="Équipe B">
      </div>
  ...
  <div class="footer">Outil — Sauvegarde automatique (localStorage) — Export CSV / Excel / PDF</div>
</div>
...
let data = {
  teamA: { name: 'Équipe A', players: [] },
  teamB: { name: 'Équipe B', players: [] },
  period: 1,
  scoreA: 0,
  scoreB: 0
};
...
/* Load / Save */
function save() { localStorage.setItem(STORAGE_KEY, JSON.stringify(data)); flashBtn('saveBtn','Sauvegardé'); }
function load(){
...
function addPlayer(team){
  ...
  if(!name || !num){ alert('Nom et numéro requis'); return; }
  const p = {
...
function renderTeam(team){
  ...
  players.forEach((p, i) => {
    ...
    div.innerHTML = `
      <h4>${p.number} — ${p.name} <span style="font-weight:700;color:#374151">Points: ${p.points}</span></h4>
      <div class="stats">1pt: ${p.shots1Made}/${att1} (${pct1}%) • 2pt: ${p.shots2Made}/${att2} (${pct2}%) • 3pt: ${p.shots3Made}/${att3} (${pct3}%)</div>
      <div class="controls-wrap">
        <!-- 1pt -->
        <button class="icon-btn positive" title="+1 marqué" onclick="adjustShot('${team}',${i},1,1,0)">➕✔</button>
        <button class="icon-btn neutral" title="-1 marqué" onclick="adjustShot('${team}',${i},1,-1,0)">➖✔</button>
        <button class="icon-btn negative" title="+1 raté" onclick="adjustShot('${team}',${i},1,0,1)">➕✖</button>
        <button class="icon-btn neutral" title="-1 raté" onclick="adjustShot('${team}',${i},1,0,-1)">➖✖</button>

        <!-- 2pt -->
        <button class="icon-btn positive" title="+2 marqué" onclick="adjustShot('${team}',${i},2,1,0)">➕✔</button>
        <button class="icon-btn neutral" title="-2 marqué" onclick="adjustShot('${team}',${i},2,-1,0)">➖✔</button>
        <button class="icon-btn negative" title="+2 raté" onclick="adjustShot('${team}',${i},2,0,1)">➕✖</button>
        <button class="icon-btn neutral" title="-2 raté" onclick="adjustShot('${team}',${i},2,0,-1)">➖✖</button>

        <!-- 3pt -->
        <button class="icon-btn positive" title="+3 marqué" onclick="adjustShot('${team}',${i},3,1,0)">➕✔</button>
        <button class="icon-btn neutral" title="-3 marqué" onclick="adjustShot('${team}',${i},3,-1,0)">➖✔</button>
        <button class="icon-btn negative" title="+3 raté" onclick="adjustShot('${team}',${i},3,0,1)">➕✖</button>
        <button class="icon-btn neutral" title="-3 raté" onclick="adjustShot('${team}',${i},3,0,-1)">➖✖</button>

        <!-- other stats -->
        <button class="icon-btn positive" title="+Rebond" onclick="adjustStat('${team}',${i},'rebounds',1)">➕Rebond</button>
        <button class="icon-btn neutral" title="-Rebond" onclick="adjustStat('${team}',${i},'rebounds',-1)">➖Rebond</button>
        <button class="icon-btn positive" title="+Passe" onclick="adjustStat('${team}',${i},'assists',1)">➕Passe</button>
        <button class="icon-btn neutral" title="-Passe" onclick="adjustStat('${team}',${i},'assists',-1)">➖Passe</button>
        <button class="icon-btn positive" title="+Interception" onclick="adjustStat('${team}',${i},'interceptions',1)">➕Interception</button>
        <button class="icon-btn neutral" title="-Interception" onclick="adjustStat('${team}',${i},'interceptions',-1)">➖Interception</button>
        <button class="icon-btn negative" title="+Faute" onclick="adjustStat('${team}',${i},'fouls',1)">➕Faute</button>
        <button class="icon-btn neutral" title="-Faute" onclick="adjustStat('${team}',${i},'fouls',-1)">➖Faute</button>
...
function buildSummaryHTML(){
  ...
  ['A','B'].forEach(team => {
    ...
    html += `<div style="background:#fff;padding:8px;border-radius:8px;color:#111"><h4>${t.name} — Score: ${data['score'+team]}</h4>`;
...
document.getElementById('exportPdf').addEventListener('click', async ()=>{
  ...
  doc.setFontSize(14);
  doc.text(`${data.teamA.name} vs ${data.teamB.name} — Résumé`, 40, 40);
...
document.getElementById('resetBtn').addEventListener('click', ()=>{
  if(!confirm('Commencer un nouveau match ? Toutes les données locales seront effacées.')) return;
  data = { teamA: { name:'Équipe A', players:[] }, teamB: { name:'Équipe B', players:[] }, period:1, scoreA:0, scoreB:0 };
